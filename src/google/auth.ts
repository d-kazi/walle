...
/**
 * Four OAuth principals. Two are dedicated accounts Wall-E may read in
 ...
 *
 *   school — the dedicated school inbox. Only ever receives BISR mail.
 *   walle  — the dedicated forwarding inbox. Also holds the shared family
 *            Drive folder, so drive.readonly covers an otherwise empty
 *            Drive rather than a personal one; drive.file limits writes to
 *            files this service created (the backups).
 ...
 */
export type Principal = User | 'school' | 'walle';

export const PRINCIPALS: readonly Principal[] = ['dan', 'alina', 'school', 'walle'] as const;
